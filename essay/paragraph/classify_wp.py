...
exp_to_data = pickle.load(open("../../writing_prompts/paragraph/symbolic_data", "rb"))

data = []
y = []
for idx in tqdm.tqdm(range(1798)):
    data.append(t_featurize(f"../../writing_prompts/data/paragraph/{idx}.txt"))
    with open(f"../../writing_prompts/data/paragraph/labels/{idx}.txt") as f:
        source = f.read().strip()
        if source == "gpt":
            y.append(1)
        else:
